...
def generate_sheet(unitname, datestart = 'now', dateend = 'now'):
    ...
    Result = {
        ...
        'I19' : f"{datestart} {starttime} to {dateend} {endtime}",
...
def generate_home(s1, s2):
    ...
    home = {
        ...
        'L19': s1[mw_loc],
        'L20': '-',
        'L21': s1['AD59'] * s1['U55'] / 100, # Waktu disable oleh operator
        'L23': s1['U54'],
        ...
        'Y19': s2[mw_loc],
        'Y20': '-',
        'Y21': round(s2['AD59'] * s2['U55'] / 100), # TODO: Waktu disable oleh operator
        'Y23': s2['U54'],

        'G30': s1['U23'] + s2['U23'],
        'G31': s1['U54'] + s2['U54'],
        'G33': f"{round(100 * (s1['U23'] + s2['U23']) / (s1['U54'] + s2['U54']),2)} %"
    }
...
def generate_report(datestart = 'now', dateend = '', kind="Excel"):
    if dateend == '' or dateend == None: 
        dateend = pd.to_datetime(datestart).strftime('%Y-%m-%d 23:59')
    ...
    if kind == "Excel":
        ...
        filesrc = 'src/Perhitungan Kertas Kerja BATTJA.xlsx'
...
if __name__ == '__main__':
    print(generate_report('2022-04-12 08:00', '2022-04-15 07:59'))